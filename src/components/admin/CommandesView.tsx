...
export default function CommandesView({
  ...
  const visibleOrders = localStatusState === "aujourd_hui" ? section1
    : localStatusState === "retard"     ? section2
    : localStatusState === "confirme"   ? section3
    : sectionHist
      ...
      <div style={{ display: "flex", gap: 6, overflowX: "auto", marginBottom: 16, paddingBottom: 4 }}>
        {[
          ...
          <button key={tab.id} onClick={() => setLocalStatusState(tab.id)}
            style={{ padding: "9px 14px", border: `1px solid ${localStatusState === tab.id ? tab.color : "#2a2a3e"}`, borderRadius: 20, cursor: "pointer", fontSize: 13, fontWeight: localStatusState === tab.id ? 700 : 400, whiteSpace: "nowrap", flexShrink: 0,
              background: localStatusState === tab.id ? tab.bg : "#111118",
              color: localStatusState === tab.id ? tab.color : "#6b7280" }}>
            {tab.label} {tab.count > 0 && <span style={{ marginLeft: 4, background: localStatusState === tab.id ? tab.color : "#2a2a3e", color: localStatusState === tab.id ? "#000" : "#9ca3af", padding: "1px 7px", borderRadius: 20, fontSize: 11 }}>{tab.count}</span>}
          </button>
        ))}
      </div>
