...
export default function AdminPage() {
  ...
  const initPage = async () => {
    ...
    if (profilesData) {
      ...
      setDrivers(allProfiles.filter((p) => normalizeRole(p.role) === "livreur"))
      setClosers(allProfiles.filter((p) => normalizeRole(p.role) === "closureuse"))
    }
    ...
    const { data: stockData } = await supabase
      .from("driver_stock").select("*").order("id", { ascending: false })
    setDriverStocks((stockData as DriverStock[]) || [])

    const { data: historyData } = await supabase
      .from("order_history").select("*").order("created_at", { ascending: false })
  ...
  const commissionStats = useMemo(() => {
    ...
    orders.forEach((o) => {
      if (o.status === "Livré" && o.driver_commission && o.driver_commission > 0) {
    ...
    orders.forEach((o) => {
      if (o.closer_id && o.closer_commission && o.closer_commission > 0) {
        const key = o.closer_id
        const closerProfile = closers.find((c) => c.id === key)
        const name = closerProfile?.full_name || "Closureuse"
    ...
    const totalDriverCommissions = Object.values(byDriver).reduce((s, v) => s + v.total, 0)
    const totalCloserCommissions = Object.values(byCloser).reduce((s, v) => s + v.total, 0)

    return { byDriver, byCloser, totalDriverCommissions, totalCloserCommissions }
  }, [orders, closers])

  const visibleOrders = useMemo(() => {
    return orders.filter((order) => {
      const matchesStatus = statusFilter === "Tous" ? true : (order.status || "En attente") === statusFilter
      const q = search.trim().toLowerCase()
      const matchesSearch = q === "" ? true :
        [order.customer_name, order.phone, order.city, order.driver_name || "",
          order.product, order.logistic_status || "", order.payment_status || "",
          order.delivery_type || "", String(order.amount || ""), String(order.quantity || "")]
  ...
  const getOrderHistory = (orderId: number) =>
    history.filter((item) => item.order_id === orderId)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      if (createdOrder) {
        await addHistory(createdOrder.id, "commande_creee",
  ...
  const handleAddStock = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!productName || quantityToAdd <= 0) { alert("Produit ou quantité invalide."); setStockLoading(false); return }
    ...
    if (existing) {
      ...
      if (error) { alert("Erreur stock : " + error.message); setStockLoading(false); return }
      setDriverStocks((prev) => prev.map((item) => item.id === existing.id ? { ...item, quantity: newQty } : item))
    } else {
      const { data, error } = await supabase.from("driver_stock").insert([{
        driver_id: driver.id, driver_name: driver.full_name, product_name: productName, quantity: quantityToAdd,
      }]).select()
      if (error) { alert("Erreur stock : " + error.message); setStockLoading(false); return }
  ...
  const updateStatus = async (id: number, newStatus: string) => {
    ...
    if (error) { alert("Erreur mise à jour : " + error.message); return false }
    setOrders((prev) => prev.map((order) => order.id === id ? { ...order, status: newStatus } : order))
    await addHistory(id, "statut_modifie", `Statut changé en ${newStatus}`)
    return true
  }

  const updatePaymentAndCash = async (id: number, newPaymentStatus: string, collected: boolean) => {
    const payload = {
    ...
    await addHistory(id, "paiement_modifie", `Paiement changé en ${newPaymentStatus}`)
    return true
  }
  ...
  const consumeDriverStock = async (order: Order) => {
    ...
    setDriverStocks((prev) => prev.map((item) => item.id === stockItem.id ? { ...item, quantity: newQty } : item))
    return true
  }
  ...
  const getAvailableActions = (order: Order) => {
    ...
    actions.push({ value: "assigner_livreur", label: "Assigner un livreur" })
    return actions
  }
  ...
  return (
    <main className="page">
      <div className="container">
        <div className="topBar">
          <div>
            ...
            <p className="subText">Connecté : <b>{profile?.full_name}</b> — rôle : <b>{profile?.role}</b></p>
          </div>
          <div className="topActions">
            <a href="/dashboard" className="linkBtn">Dashboard</a>
            <button onClick={handleLogout} className="logoutBtn">Déconnexion</button>
          </div>
        </div>

        <div className="menuBar">
          {["commandes", "creer", "stock", "commissions"].map((view) => (
            <button key={view} className={`menuBtn ${activeView === view ? "active" : ""}`} onClick={() => setActiveView(view)}>
              {view === "commandes" && "📦 Commandes"}
              {view === "creer" && "➕ Nouvelle commande"}
              {view === "stock" && "📦 Stock livreurs"}
        ...
        {activeView === "commandes" && (
          <section className="panel">
            ...
            <div className="filters">
              ...
              <input type="text" placeholder="Recherche nom, téléphone, ville, livreur, produit"
                value={search} onChange={(e) => setSearch(e.target.value)} className="field" />
            </div>
            <p className="countText">{visibleOrders.length} commande(s) trouvée(s)</p>
                ...
                {visibleOrders.map((order) => {
                  ...
                  return (
                    <article key={order.id} className="orderCard">
                      ...
                      <p><b>Commission livreur :</b> {order.driver_commission ? formatMoney(order.driver_commission) : "—"}</p>
                      <p><b>Commission closureuse :</b> {order.closer_commission ? formatMoney(order.closer_commission) : "—"}</p>
                      <div className="badgesWrap">
                        <div className="statusBadge" style={{ background: getStatusColor(order.status) }}>Global : {order.status || "En attente"}</div>
                        <div className="statusBadge" style={{ background: getLogisticColor(order.logistic_status) }}>Logistique : {order.logistic_status || "En attente"}</div>
                        <div className="statusBadge" style={{ background: getPaymentColor(order.payment_status) }}>Paiement : {order.payment_status || "Non payé"}</div>
                      ...
                      <div className="cleanActionsBox">
                        ...
                        <select value={selectedDrivers[order.id] || ""} onChange={(e) => handleDriverSelect(order.id, e.target.value)} className="field">
                          <option value="">Choisir un livreur</option>
                          {drivers.map((driver) => (
                            <option key={driver.id} value={driver.id}>{driver.full_name} — {driver.email}</option>
                          ))}
                        </select>
                        <button type="button" onClick={() => applyOrderAction(order)} className="applyBtn">Appliquer</button>
                      </div>
                      <div className="historyBlock">
                        <h3 className="historyTitle">Historique</h3>
                        {getOrderHistory(order.id).length === 0 ? (
                          <p className="historyEmpty">Aucune action enregistrée.</p>
                        ) : (
                          <div className="historyList">
                            {getOrderHistory(order.id).map((item) => (
                              <div key={item.id} className="historyItem">
                                <p><b>{item.action_by_name}</b> — {item.action_type}</p>
                                <p>{item.action_details}</p>
                                <p className="historyDate">{new Date(item.created_at).toLocaleString()}</p>
                              </div>
                            ))}
                          </div>
                        )}
        ...
        {activeView === "creer" && (
          <section className="panel">
            ...
            <form onSubmit={handleSubmit} className="form">
              ...
              <button type="submit" disabled={loading} className={`submitBtn ${loading ? "isLoading" : ""}`}>
        ...
        {activeView === "stock" && (
          <section className="panel">
            <h2 className="sectionTitle">Ajouter du stock à un livreur</h2>
            <form onSubmit={handleAddStock} className="form">
              ...
              <input name="product_name" placeholder="Nom du produit" value={stockForm.product_name} onChange={handleStockChange} required className="field" />
              <input name="quantity" type="number" min="1" placeholder="Quantité à donner" value={stockForm.quantity} onChange={handleStockChange} required className="field" />
              <button type="submit" disabled={stockLoading} className={`submitBtn ${stockLoading ? "isLoading" : ""}`}>
                {stockLoading ? "Ajout..." : "Ajouter le stock"}
              </button>
            </form>
            <div className="stockBlock">
              <h2 className="sectionTitle" style={{ marginTop: "24px" }}>Stock actuel</h2>
        ...
        {activeView === "commissions" && (
          <section className="panel">
            <h2 className="sectionTitle">💰 Commissions</h2>
            <div className="commissionTotals">
              <div className="totalCard green">
                <span className="totalLabel">Total commissions livreurs</span>
                <span className="totalValue">{formatMoney(commissionStats.totalDriverCommissions)}</span>
              </div>
              <div className="totalCard blue">
                <span className="totalLabel">Total commissions closeuses</span>
                <span className="totalValue">{formatMoney(commissionStats.totalCloserCommissions)}</span>
              </div>
            </div>
              ...
              <div className="commissionGrid">
                {Object.values(commissionStats.byDriver).map((item, i) => (
                  <div key={i} className="commissionCard">
                    ...
                    <span className="commissionCount">{item.count} commande(s) livrée(s)</span>
                    <span className="commissionAmount green">{formatMoney(item.total)}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <style jsx>{`
        ...
        .linkBtn, .logoutBtn { padding: 12px 16px; border-radius: 12px; border: none; text-decoration: none; color: white; font-weight: bold; cursor: pointer; display: inline-block; }
        .linkBtn { background: #2563eb; }
        ...
        .sectionTitle { margin-top: 0; margin-bottom: 14px; font-size: 28px; }
        .subTitle { font-size: 18px; margin: 20px 0 12px; }
        .form { display: flex; flex-direction: column; gap: 12px; max-width: 560px; }
        .stockBlock { border-top: 1px solid #475569; margin-top: 24px; padding-top: 24px; }
        .field { width: 100%; padding: 14px 16px; border-radius: 12px; border: 1px solid #475569; background: #0f172a; color: white; outline: none; font-size: 16px; box-sizing: border-box; }
        .submitBtn { padding: 15px; border-radius: 12px; border: none; background: #22c55e; color: white; font-weight: bold; cursor: pointer; font-size: 18px; }
        .submitBtn.isLoading { background: #64748b; cursor: not-allowed; }
        .filters { display: grid; grid-template-columns: 200px 1fr; gap: 10px; margin-bottom: 14px; }
        .countText, .emptyText { opacity: 0.9; margin-top: 0; }
        .ordersList { display: grid; gap: 14px; }
        .orderCard { background: #334155; padding: 16px; border-radius: 16px; }
        .orderCard p { margin: 0 0 8px 0; font-size: 16px; line-height: 1.45; }
        ...
        .applyBtn { padding: 14px 16px; border-radius: 12px; border: none; background: #7c3aed; color: white; font-weight: bold; cursor: pointer; font-size: 16px; }
        .historyBlock { margin-top: 18px; padding-top: 14px; border-top: 1px solid #475569; }
        .historyTitle { margin: 0 0 10px 0; font-size: 18px; }
        .historyEmpty { opacity: 0.85; margin: 0; }
        .historyList { display: grid; gap: 10px; }
        .historyItem { background: #1e293b; border-radius: 12px; padding: 12px; }
        .historyItem p { margin: 0 0 6px 0; font-size: 14px; line-height: 1.4; }
        .historyDate { opacity: 0.75; font-size: 13px; }
        .commissionTotals { display: grid; grid-template-columns: 1fr 1fr; gap: 14px; margin-bottom: 24px; }
        ...
        .commissionGrid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; }
        .commissionCard { background: #334155; border-radius: 16px; padding: 18px; display: flex; flex-direction: column; gap: 8px; }
        .commissionName { font-size: 18px; font-weight: bold; }
        .commissionCount { font-size: 14px; opacity: 0.8; }
        .commissionAmount { font-size: 22px; font-weight: bold; }
        .commissionAmount.green { color: #4ade80; }
        .commissionAmount.blue { color: #60a5fa; }
        @media (max-width: 980px) {
          .cleanActionsBox, .whatsappRow, .commissionTotals { grid-template-columns: 1fr; }
          .filters { grid-template-columns: 1fr; }
          .menuBar { display: grid; grid-template-columns: 1fr 1fr; }
        }
